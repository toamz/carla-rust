...
public:
  ...
  GetWaypoint(const FfiLocation &location, bool project_to_road = true,
              int32_t lane_type =
                  static_cast<uint32_t>(Lane::LaneType::Driving)) const {
    auto ptr =
        inner_->GetWaypoint(location.as_native(), project_to_road, lane_type);
    if (ptr == nullptr) {
      return nullptr;
    } else {
  ...
  std::shared_ptr<FfiWaypoint> GetWaypointXODR(RoadId road_id, LaneId lane_id,
                                               float s) const {
